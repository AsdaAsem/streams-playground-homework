...
public class Homework1 {
    ...
    public boolean streamPipeline1() {
        ...
        return countries.stream().anyMatch(x -> x.getName().toLowerCase().contains("island"));
    ...
    public Optional<String> streamPipeline2() {
        return countries.stream().filter(a -> a.getName().toLowerCase().contains("island")).findFirst().map(Country::getName);
    }
    ...
    public void streamPipeline3() {
        countries.stream()
                .map(Country::getName)
                .filter(a -> a.toLowerCase().charAt(0) == a.charAt(a.length() - 1))
    ...
    public void streamPipeline8() {
        countries.stream().sorted(Comparator.comparing(country -> country.getTimezones().size()))
                .forEach(country -> System.out.println( "@" + country.getCode() + " " + country.getName()+" : "+country.getTimezones().size()));
    }
